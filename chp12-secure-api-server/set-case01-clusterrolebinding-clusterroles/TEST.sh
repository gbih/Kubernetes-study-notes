...
#  echo "kubectl exec -it my-pod -n chp12-set1227 -- curl localhost:8001/api/v1/componentstatuses"
  ...
  # kubectl exec -it my-pod -n=chp12-set1227 -- curl http://localhost:8001/apis/admissionregistration.k8s.io/v1/validatingwebhookconfigurations
   kubectl exec -it my-pod -n=chp12-set1227 -- curl http://localhost:8001/apis/certificates.k8s.io/v1beta1/certificatesigningrequests
  # kubectl exec -it my-pod -n=chp12-set1227 -- curl http://localhost:8001/apis/authentication.k8s.io/v1/tokenreviews
...
#kubectl create clusterrolebinding my-binding --clusterrole=system:auth-delegator --serviceaccount=chp12-set1227:my-sa --dry-run=client -o yaml > crb-1.yaml
