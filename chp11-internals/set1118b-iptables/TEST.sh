...
. ~/src/common/setup.sh
FULLPATH=$(pwd)
echo "11.1.8b Extra: Exploring iptables"
echo $HR_TOP
...
echo "kubectl rollout status deployment hostnames -n=chp11-set1118b"
kubectl rollout status deployment hostnames -n=chp11-set1118b
...
echo "kubectl get all -n chp11-set1118b -L app"
kubectl get all -n chp11-set1118b -L app


echo $HR

echo "kubectl get pods -n chp11-set1118b -o go-template='{{range .items}}{{.status.podIP}}{{"\n"}}{{end}}'"
kubectl get pods -n chp11-set1118b -o go-template='{{range .items}}{{.status.podIP}}{{"\n"}}{{end}}'
