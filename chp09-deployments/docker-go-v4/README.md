...
## Build Docker image, tag as `kubia:v4', push to Docker Hub

```
docker build -t georgebaptista/kubia:v4 .
docker push georgebaptista/kubia:v4
```

## Run
```
docker run --name kubia-container -p 8080:8080 -d georgebaptista/kubia:v4
```
...
docker rmi georgebaptista/kubia:v4
docker image prune
```
