...
. ~/src/common/setup.sh
FULLPATH=$(pwd)
echo "16.3.1 Using inter-pod affinity to deploy pods on the same node"
echo $HR_TOP
...
echo "kubectl wait --for=condition=Ready=True pods/curl-restrictive -n=chp16-set1631 --timeout=10s"
kubectl wait --for=condition=Ready=True pods/curl-restrictive -n=chp16-set1631 --timeout=10s
