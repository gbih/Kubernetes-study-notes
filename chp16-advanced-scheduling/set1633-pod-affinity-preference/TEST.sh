...
. ~/src/common/setup.sh
FULLPATH=$(pwd)
echo "16.3.3 Expressing pod affinity preferences instead of hard requirements"
echo $HR_TOP
...
echo "kubectl wait --for=condition=Ready=True pods/curl-restrictive -n=chp16-set1633 --timeout=10s"
kubectl wait --for=condition=Ready=True pods/curl-restrictive -n=chp16-set1633 --timeout=10s

enter

echo "kubectl rollout status deployment backend -n=chp16-set1633"
kubectl rollout status deployment backend -n=chp16-set1633
...
echo "kubectl exec -it curl-restrictive -n chp16-set1633 -- curl localhost:8001/api/v1/namespaces/chp16-set1633/services | jq 'del(.metadata.uid, .items[].metadata.managedFields, .items[].metadata.annotations, .status)' | yq r -P -"
echo ""

kubectl exec -it curl-restrictive -n chp16-set1633 -- curl localhost:8001/api/v1/namespaces/chp16-set1633/services | jq 'del(.metadata.uid, .items[].metadata.managedFields, .items[].metadata.annotations, .status)' | yq r -P -
