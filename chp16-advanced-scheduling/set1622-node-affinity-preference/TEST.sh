...
echo "kubectl rollout status deployment kubia-deploy -n=chp16-set1622"
kubectl rollout status deployment kubia-deploy -n=chp16-set1622
...
echo "kubectl exec -it curl-restrictive -n chp16-set1622 -- curl localhost:8001/api/v1/namespaces/chp16-set1622/services | jq 'del(.metadata.uid, .items[].metadata.managedFields, .items[].metadata.annotations, .status)' | yq r -P -"
echo ""

kubectl exec -it curl-restrictive -n chp16-set1622 -- curl localhost:8001/api/v1/namespaces/chp16-set1622/services | jq 'del(.metadata.uid, .items[].metadata.managedFields, .items[].metadata.annotations, .status)' | yq r -P -
